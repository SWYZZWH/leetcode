...
class Solution:
    ...
    # use heap might faster, but the time complexity is always O(n log n)
    def reorderLogFiles(self, logs: List[str]) -> List[str]:
        ...
        def letter_log_sort_key(letter: str):
            for i, s in enumerate(letter):
                if s == " ":
                    return chr(1).join([letter[i+1:], letter[:i]])

        ret = sorted(ret, key=letter_log_sort_key)
